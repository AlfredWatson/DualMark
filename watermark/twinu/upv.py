...
class UPVConfig(BaseConfig):
    ...
    def __init__(
        ...
        self.gamma_upv = self.config_dict['gamma']
        self.delta_upv = self.config_dict['delta']
        self.z_threshold_upv = self.config_dict['z_threshold']
        self.prefix_length_upv = self.config_dict['prefix_length']
...
class UPVLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if input_ids.shape[-1] < self.config.prefix_length_upv:
            return scores

        green_tokens_mask = torch.zeros_like(scores)
        for b_idx in range(input_ids.shape[0]):
            greenlist_ids = self.utils.get_greenlist_ids_upv(input_ids[b_idx], scores=scores[b_idx])
            green_tokens_mask[b_idx][greenlist_ids] = 1
        green_tokens_mask = green_tokens_mask.bool()

        scores = self._bias_greenlist_logits(
            scores=scores, greenlist_mask=green_tokens_mask, greenlist_bias=self.config.delta_upv
        )
...
class UPV(BaseWatermark):
    ...
    def __init__(self, algorithm_config: UPVConfig) -> None:
        """
            Initialize the UPV algorithm.

            Parameters:
                algorithm_config (UPVConfig): UPVConfig instance.
        """
        utils = UPVUtils(algorithm_config)
        logits_processor = UPVLogitsProcessor(algorithm_config, utils)
    ...
    #         encoded_watermarked_text[:, prompt_length:],
    ...
    def detect_watermark(self, text: str, return_dict: bool = True, *args, **kwargs):
        ...
        if self.config.detect_mode_upv == 'key':
            ...
            is_watermarked = z_score > self.config.z_threshold_upv
        else:  # network
